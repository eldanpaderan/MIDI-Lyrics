...
## Phase 15 — Module Loading / 404 Investigation

**Date:** 2026-07-26

**Phase:** Investigate a reported GitHub Pages 404 for `services/firebase/browser-bridge.js`, per an explicit, detailed verification checklist. No code was assumed broken — every item was independently checked before concluding anything.

**Verification performed (all items from the request):**
1. `index.html`'s `<script type="module">` reference — inspected directly: `src="services/firebase/browser-bridge.js"`.
2. `browser-bridge.js` existence — confirmed present at the exact expected path.
3–4. Moved/renamed file — not applicable; the file is at its original, unchanged location and name.
5–6. Every ES Module import path and relative path across all 8 `services/firebase/*.js` files — cross-checked programmatically against actual filenames; all resolve.
7. GitHub Pages deployment paths — the script tag uses a root-relative-from-document path with no leading slash, which resolves correctly regardless of whether the site is served from a domain root or a project subpath (as GitHub Pages project sites typically are).
8. Case-sensitive filenames — checked every cross-import's exact casing against the actual filename on disk (GitHub Pages, being Linux-based, is case-sensitive, unlike Windows/macOS' default filesystems) — all match exactly; no `Browser-Bridge.js`/`BROWSER-BRIDGE.js`/similar variant exists anywhere.
9. `browser-bridge.js`'s imports of `firebase.js`/`auth.js`/`session.js`/`realtime.js`/`preference.js` — confirmed correct (via `index.js`'s barrel export, which `browser-bridge.js` imports).
10. `window.MLFirebase` exposure — confirmed present (`window.MLFirebase = MLFirebase;`), synchronous, immediately upon module evaluation.
11. `app.js` waiting for initialization — confirmed already fixed in Phase 14 (the `MLFirebaseReady` event–based wait in `ensureFirebaseReady()`).
12. Hidden/non-ASCII characters in the script tag's path string — checked via a byte-level dump; confirmed plain ASCII, no BOM, no smart quotes, no stray whitespace.
13. `.gitignore` — none exists in the repository, so nothing is being excluded from version control on that front.
14. Duplicate or case-mismatched `services` directories — none found; exactly one, correctly cased.

**Finding: no missing module, no broken path, and no incorrect import were found anywhere in this repository.** Every file, path, import, and export checked resolves correctly and matches byte-for-byte. This specific 404 could not be reproduced or explained from the repository content available in this environment.

**Most likely explanation (deployment-side, not code-side):** since files are delivered here as downloads for you to place into your actual GitHub repository (this environment has no direct push access to it), a 404 specifically on your live GitHub Pages site for a file that is structurally correct here most likely means one of:
- The `services/firebase/` folder (or some files within it) was not yet committed/pushed to the live repository.
- A local case-insensitive filesystem (common on Windows/macOS) allowed a case mismatch to exist locally without git noticing, which only surfaces once served by GitHub Pages' case-sensitive Linux hosting.
- GitHub Pages' build/propagation had not yet completed after a recent push (typically resolves within a few minutes).
- An older commit/branch is what's actually configured as the Pages source, predating the `services/` folder's introduction.

**Files Modified:**
- `.nojekyll` (new, empty file at the repository root) — GitHub Pages runs a Jekyll build by default unless this file is present. While no folder in this project starts with `_` (Jekyll's usual exclusion trigger) so this is unlikely to be the actual cause here, adding `.nojekyll` is the standard, zero-risk hardening step recommended for any static JS site on GitHub Pages, and directly relevant to "verify GitHub Pages deployment paths." This does not change any application behavior.

**Files NOT modified:** every other file — `index.html`, `app.js`, and all of `services/firebase/*.js` — was verified correct as-is; per the explicit instruction not to modify unrelated code, nothing else was touched.

**Recommended verification checklist for your live repository (outside this environment's visibility):**
1. Confirm `services/firebase/browser-bridge.js` (and its 7 sibling files) actually appear in your GitHub repository's file browser, at that exact path and casing.
2. Confirm your repository's Pages settings (Settings → Pages) point at the branch/folder that actually contains these files.
3. Hard-refresh (or open in an incognito window) to rule out a stale cached 404 from before a fix was pushed.
4. If you renamed/moved any file locally on Windows or macOS, run `git status`/`git mv` explicitly to confirm git registered the correct casing, since case-only renames are a well-known gotcha on case-insensitive local filesystems.

**Startup sequence — confirmed correct in the code, pending your live-deployment verification:**
`index.html` → `browser-bridge.js` loads (module, deferred) → `initFirebase()`/`initAuth()`/`initPreferenceSync()` run (opt-in, config-gated) → `window.MLFirebase` exists → toggling "Enable real-time sync" calls `ensureFirebaseReady()` (now wait-capable, per Phase 14) → `syncSessionForCurrentMode()` creates/joins the shared session → Leader publishes via `realtime.js` → Followers consume via `watchPlaybackState`/`watchDisplayState` and re-render, never echoing back.

**Known Limitation:** this environment cannot access your live GitHub Pages URL or repository directly, so the actual 404 could not be reproduced or confirmed fixed here — the conclusion above (repository content is structurally correct) is based on a complete, direct inspection of every file and path involved, not a live network test.

---

## Phase 16 — Repository Restructuring (Full Modular Decomposition)

**Date:** 2026-07-27

**Phase:** A major, explicitly-approved restructuring of the entire repository into the target folder structure (`services/{firebase,library,midi,ui,utils}/`). Before starting, I flagged that (a) no actual file-placement bug existed (re-verified three times across two prior turns — `services/firebase/browser-bridge.js` was always correctly placed), and (b) the requested target structure required splitting `app.js` (a ~1800-line classic script) into multiple ES modules, which is a genuine architectural redesign that conflicted with "do not redesign"/"do not rewrite working functionality" stated in the same request. You explicitly confirmed you wanted the full restructuring done anyway, acknowledging the tradeoff — this phase proceeds on that basis.

### Every File Moved
| From | To |
|---|---|
| `services/firebase/library.js` | `services/library/library.js` (relative imports to `firebase.js`/`auth.js` updated to `../firebase/...`) |

### Every File Renamed
None — no file was renamed; `app.js` keeps its name and location, but its *role* changed (see below).

### New Files Created (all content extracted verbatim from the old monolithic `app.js`, not rewritten)
- `services/library/parser.js` — the `[PAGE]`/blank-line page-splitting heuristic, extracted into ONE shared function (`parseLyricsIntoPages`) for the first time — this resolves the duplicated-parsing tech debt flagged since the very first architecture audit (originally duplicated across `loadLyrics()` and the now-removed `handleFollowerUpdate()`; by Phase 13 it existed only in `applySongAndRender()`, but remained inline/unconsolidated until now).
- `services/library/metadata.js` — `formatSongName()`, `formatTimestamp()`.
- `services/midi/bindings.js` — `midiDesc()`, plus two new small pure helpers (`midiKey()`, `matchesBinding()`) extracted from what was previously inline logic in `onMIDIMessage()`, to give the note/CC-matching logic a name and a single home.
- `services/midi/learn.js` — `toggleMidiLearn()`, `clearMidiMapping()`, `updateMidiMappingInfo()`.
- `services/midi/midi.js` — `initMIDI()`, `onMIDIMessage()`.
- `services/ui/viewer.js` — the shared `state` object, `FONT_SIZES`, `loadLyrics()`, `applySongAndRender()` (now calling `parseLyricsIntoPages()` instead of inlining the split logic), `renderPage()`, `navigate()`, `updateNavButtons()`, `updatePageCounter()`, `renderPips()`, `updatePips()`, `changeFontSize()`, `applyFontSizeByLabel()`, `loadFontSizeFromPreferences()`, `updateFontDisplay()`, auto-fit (`initAutoFit()`, `toggleAutoFit()`, `fitLyricText()` — the resize handler now uses the new generic `debounce()` utility instead of an ad-hoc `setTimeout`/`clearTimeout` pattern), the desktop keyboard shortcuts (arrows/space), and the touch/swipe listeners.
- `services/ui/sidebar.js` — `loadSongList()`, `detectGitHubRepo()`, `fetchSongsFromGitHubAPI()`, `fetchSongsFromManifest()`, `fetchSongsFromDirectory()`, `renderSetlist()`, `selectSong()`, `setMode()`.
- `services/ui/toolbar.js` — `setPillState()`, Theme (`initTheme`, `applyThemeToDOM`, `setTheme`, `cycleTheme`), Fullscreen (`toggleFullscreen`, `applyFullscreenState`, `updateFullscreenBtn`, `initFullscreenAutoCollapse`), Sidebar collapse (`initSidebarCollapse`, `toggleSidebarCollapse`, `updateSidebarCollapseBtn`), Wake Lock (`toggleWakeLock`, `acquireWakeLock`, `releaseWakeLock`), Drawer (`toggleDrawer`, `closeSidebarDrawer`), and the toolbar-related keyboard shortcuts (f/t/[/Escape).
- `services/ui/settings.js` — `MAIN_SESSION_ID`, the Firebase config modal functions, `toggleFirebase()`, `ensureFirebaseReady()`/`proceedWithFirebaseReady()`, `syncSessionForCurrentMode()`, `stopModularSync()`, `publishCurrentSongIfLeader()`/`publishPageIfLeader()`/`publishDisplayIfLeader()`, `playSession()`/`pauseSession()`/`stopSession()`/`updatePlayPauseStopUI()`, the subscription lifecycle (`ensurePlaybackSubscription`/`ensureDisplaySubscription`/`teardownSubscriptions`), and `handleIncomingPlaybackState()`/`handleIncomingDisplayState()` — this is the "orchestration layer" the Complete Migration phase described, now with an explicit home.
- `services/ui/dialogs.js` — the entire Cloud Song Library modal (Songs/Collections/Playlists/Recent tabs) and Lyrics Editor. `libraryCache` is exported as a live binding since `settings.js` needs it. The editor's auto-save now uses the shared `debounce()` utility. Added one small new function, `openEditingSongOnStage()` — see "Fix required by the restructuring" below.
- `services/utils/storage.js` — `saveLocalPrefs()`/`loadLocalPrefs()`, `getSyncedPref()`/`setSyncedPref()`.
- `services/utils/debounce.js` — new generic `debounce(fn, waitMs)` utility, now used by both the auto-fit resize handler and the editor auto-save (previously two separate ad-hoc `setTimeout`/`clearTimeout` implementations).
- `services/utils/helpers.js` — `showToast()`, `escapeHtml()`, `detectPlatform()`.
- `app.js` (rewritten as a thin entry point) — imports every submodule, exposes the ~30 functions referenced by inline HTML `onclick=`/`onchange=`/`oninput=` attributes onto `window` (necessary because it's now `type="module"`, and ES modules are not global by default), and runs one consolidated `DOMContentLoaded` init sequence (previously split across two separate listeners from the original UI-modernization phase — now one, same order).

### Fix Required By The Restructuring
- `index.html`'s Lyrics Editor "Open on Stage" button called `onclick="openSongFromLibrary(currentEditingSongId); closeEditorModal();"` — directly referencing `currentEditingSongId` as a bare global identifier. Since that variable is now module-scoped inside `ui/dialogs.js` (not global), this would have broken. Fixed by adding one small new exported function, `openEditingSongOnStage()`, in `dialogs.js`, and updating the HTML to call it instead of referencing the internal variable directly. This is the one small, necessary addition made to keep existing functionality working under the new module structure — not a new feature.

### Every Import That Changed
- `services/library/library.js`'s imports of `firebase.js`/`auth.js`: `'./firebase.js'`/`'./auth.js'` → `'../firebase/firebase.js'`/`'../firebase/auth.js'` (moved one directory level).
- `services/firebase/index.js`'s re-export of the Library API: `export * from './library.js'` → `export * from '../library/library.js'` — `window.MLFirebase` continues to expose every Library function exactly as before; nothing that depends on it (`ui/dialogs.js`, `ui/settings.js`) needed to change.
- Every other import in the new `services/{library,midi,ui,utils}/*.js` files is new (these files didn't exist before) — see each file's own header comment for its exact import list.

### Deliberate, Documented Circular Imports (safe under ES Module semantics)
Splitting a tightly-coupled ~1800-line file into modules surfaced 5 circular import pairs. Each was individually verified: the imported binding is used only inside a function body or event-callback (never at true module-evaluation/top-level time), which is safe under ES Module semantics (function/live-binding exports are resolved lazily, not copied at import time).
1. `utils/storage.js` ↔ `ui/sidebar.js` — `loadLocalPrefs()` calls `setMode()`; `setMode()` calls `saveLocalPrefs()`.
2. `utils/storage.js` ↔ `ui/viewer.js` — `loadLocalPrefs()` calls `loadFontSizeFromPreferences()`; `changeFontSize()`/auto-fit call `getSyncedPref()`/`setSyncedPref()`.
3. `utils/storage.js` ↔ `midi/learn.js` — `loadLocalPrefs()` calls `updateMidiMappingInfo()`; `clearMidiMapping()`/`onMIDIMessage()`(via learn.js's own persistence) call `saveLocalPrefs()`.
4. `ui/toolbar.js` ↔ `ui/settings.js` — `setTheme()`/fullscreen-change publish via `publishDisplayIfLeader()`; `handleIncomingDisplayState()` applies received updates via `applyThemeToDOM()`/`applyFullscreenState()`.
5. `ui/viewer.js` ↔ `ui/settings.js` — `navigate()`/`changeFontSize()` publish via `publishPageIfLeader()`/`publishDisplayIfLeader()`; `handleIncomingPlaybackState()` renders received updates via `applySongAndRender()`/`renderPage()`.

### Testing / Verification Performed
- `node --input-type=module --check` on **every** JS file in the repository (`app.js` + all 21 files under `services/`) — all pass.
- Full relative-import-path resolution across the entire repository (excluding comment-only false positives, manually confirmed) — every real `import ... from './...'` resolves to an actual file.
- Full named-export cross-check — every `import { x } from '...'` resolves to a real `export` in its target file.
- Confirmed every function referenced by an inline HTML `onclick=`/`onchange=`/`oninput=` attribute is both imported into and exposed on `window` by the new `app.js`.
- Full `getElementById()` cross-check across every new module file plus `app.js` against `index.html`'s `id` attributes (including known dynamically-created elements) — all resolve.
- Programmatically detected all 5 circular import pairs and manually verified each is "safe" (function-only usage, confirmed via a top-level-statement scan of every file in each pair).
- Checked for duplicate exported function names across the whole `services/` tree — none found.
- A broader heuristic scan for "function calls to names that are neither imported nor locally defined" flagged ~20 apparent hits per file; every one was manually inspected and confirmed to be either (a) an English word inside a comment coincidentally followed by a parenthetical remark (e.g. "not (yet)"), or (b) a legitimate `window.MLFirebase.methodName()` call (the established cross-module Firebase-access pattern used consistently throughout the project) — zero genuine undefined-reference bugs found.
- Confirmed `services/firebase/` no longer contains `library.js` (fully moved, not duplicated).
- Confirmed `styles.css` is untouched (hash-compared) — this restructuring only touched JS module organization and `index.html`'s two `<script>` tags (both now `type="module"`, same relative order preserved so `browser-bridge.js` still executes before `app.js`, per ES Module spec's guaranteed in-order execution of deferred/module scripts).
...
services/
  ...
  utils/
    ...
    storage.js
songs/
  Worthy Chords by Elevation Worship.txt
```
This matches the requested target structure exactly.

### Known Limitations (unchanged from prior phases, not addressed this phase)
- No live browser test was possible in this environment — all verification above is static (syntax, path resolution, export/import cross-referencing). Given the scope of this restructuring (every application file was touched or moved), a manual smoke test in a real browser is **strongly** recommended before this is considered production-verified — specifically: every button/toggle/tab in the UI, MIDI Learn, and the full Leader→Follower sync workflow across at least two devices.
- The 5 documented circular import pairs, while verified safe, are a direct consequence of how tightly-coupled the original single-file application was — this is a structural characteristic of the restructuring itself, not a new defect, but worth keeping in mind for any future module boundary changes.
- All prior known limitations (no Realtime Database security rules shipped, no automated test suite, fullscreen mirroring is best-effort on Follower devices, no mutual exclusivity for multiple Leaders) remain unchanged.

---

## Post-Restructuring Hotfix: Frozen Module Namespace Object (`services/firebase/browser-bridge.js`)

### Symptom
After the Complete Migration phase, most UI controls worked, but **Enable Realtime Sync**, **Leader**, and **Follower** did not — Firebase sync consistently failed after an 8-second timeout with "Firebase services failed to load — check your connection and refresh the page," even with a valid, saved Firebase config.

### Root Cause
`browser-bridge.js` used:
```js
import * as MLFirebase from './index.js';
window.MLFirebase = MLFirebase;
...
window.MLFirebase.ensureFirebaseServices = bootstrapFirebaseServices; // threw here
```
`import * as X` produces an ES Module namespace object, which per spec is **non-extensible**. Adding a new property to it (`ensureFirebaseServices`) throws `TypeError: Cannot add property ensureFirebaseServices, object is not extensible`. Because this throw happened at the file's top level, every line after it — including `window.MLFirebaseReadyFired = true` and the `MLFirebaseReady` `dispatchEvent` call — never ran. `services/ui/settings.js`'s `ensureFirebaseReady()` waits for exactly that property/event, so it always fell through to its timeout branch.

This was verified by actually executing the real module graph (Node's native ESM loader against a jsdom-provided `document`/`window`), not just static inspection — the thrown `TypeError` was reproduced directly.

### Fix
```js
window.MLFirebase = { ...MLFirebase };
```
Spreads the namespace object's exports into a plain, extensible object. Every existing `window.MLFirebase.xyz()` call site elsewhere in the app (`ui/settings.js`, `ui/dialogs.js`, etc.) continues to work identically — the object just happens to now accept the `ensureFirebaseServices` property being added afterward.

### Verification
- Re-ran the real module graph (Node ESM + jsdom) after the fix: no top-level errors, `typeof window.MLFirebase.ensureFirebaseServices === 'function'`.
- Re-audited every inline HTML handler (`onclick`/`onchange`/`oninput`) against `app.js`'s imports/exports/`window` assignments — all ~36 were already correct pre-fix and remain correct; this bug was unrelated to the handler-exposure layer.
- Searched the whole `services/` tree for other `import * as X` usages later mutated the same way — none found; this was the only instance of the pattern.
